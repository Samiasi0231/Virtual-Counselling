...
const CounselorAvailability = () => {
  ...
  // Helper: Remove duplicates for same start/end
  ...
  const handleAddTimeSlot = () => {
    ...
    const existing = (availability[key] || []).map(slot => ({
  ...
  const handleSave = async () => {
    const payload = Object.entries(availability).map(([date, time_slots]) => ({
      date,
      time_slots: time_slots.map(({ start_time, end_time }) => ({
        start_time,
        end_time
      })),
    }));
  ...
  return (
    <div className="sm:p-6 lg:px-10 py-6 border border-gray-200 overflow-auto mx-auto p-4 bg-white rounded-xl shadow">
      ...
      <div className="mb-6">
        ...
        />
        {tooltip && <div className="mt-2 text-sm text-red-500 italic">{tooltip}</div>}
          ...
          <div className="flex items-end space-x-4">
            ...
            <div className="flex flex-col">
              ...
              <button
                ...
                disabled={
                  !selectedDate ||
                  new Date(getFormattedDate(selectedDate)) <
